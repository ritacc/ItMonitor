...
namespace GDK.DAL.CompSearch
{
    public class PdfDayDA : DALBase
    {
        ...
        {
            string sql = string.Format(@"select channelno,monitordate,round( avg(monitorvalue),2) val,max(monitorvalue) maxval from ReportTemp
...
where bus.ParentId= {0} and dt.TypeID=10", busDeviceID);
        ...
        /// </summary>
        /// <returns></returns>
        public DataTable GetUseTableInfo(int BussID, int ChaanceNo)
        {
...
group by monitordate order by monitordate";
        ...
        public DataTable SlectDBNameSpanceUseDetail(DateTime StartTime,DateTime endTime, int BussID)
        {
...
from (
	...
	where channelno=42105 and MonitorTime >='{0} 00:00:00' and MonitorTime <='{0} 23:59:59'
	group by deviceno,channelno	
) as gro
...
(
		select deviceno,channelno,round(AVG(monitorvalue),2) monitorvalue from ReportTemp
		where channelno=42105 and MonitorTime >='{2} 00:00:00' and MonitorTime <='{2} 23:59:59'
		group by deviceno,channelno
) as sy on sy.deviceno= gro.deviceno and sy.channelno= gro.channelno
order by  DBName,tableSpaceName"
, StartTime.ToString("yyyy-MM-dd"), BussID, endTime.ToString("yyyy-MM-dd"));
            return db.ExecuteQuery(sql);
        }
...
order by monitordate", ChanncelNo);
        ...
        public DataTable SelectMZLDetail(DateTime startTime, DateTime endTime)
        {
...
left join t_Device d on hcq.deviceno= d.DeviceID", startTime.ToString("yyyy-MM-dd")
...
order by monitordate";
        ...
        public DataTable SelectOnlineDetail(DateTime startTime, DateTime endTime, int BussID)
        {
...
order by DBName"
        ...
        #region 1. 数据库连接池汇总统计
        public DataTable DBTableSpaceLineNumberDetail(DateTime startTime, DateTime endTime, int BussID)
        {
            string sql = string.Format(@"select d.DeviceName,db.DeviceName DBName, ditem.DeviceName tableSpaceName,gro.*
from (
...
order by deviceno,channelno", sysid);
	...
	order by deviceno,monitordate", deviceno);
        ...
        public DataTable MiddlewareJVMDetail(DateTime startTime, DateTime endTime)
        {
...
inner join t_Device  d on  d.DeviceID= bus.ParentId
order by deviceno,channelno ", startTime.ToString("yyyy-MM-dd")
            ,endTime.ToString("yyyy-MM-dd"));
...
where bus.ParentId= {0} and dt.TypeID=10", busDeviceID);
	...
	order by deviceno,monitordate", deviceno);
        ...
        public DataTable MiddlewareServerSessionDetail(DateTime startTime, DateTime endTime, int BussID)
        {
